...
(signal channel deliver [_r _f {droplet-address :droplet-address}]
        (let [
              ...
              ;; TODO this is here only temporarily til we get groove
              ;; translation on sending worked out 
              body (if (:message content) (:message content) (:body content))
              ]
          (--> (apply get-signal-function control-signal) _r controller-address {:command :send :params {:body body :thread (:thread content) :subject (:subject content) :type (:type content) :to to-xmpp}}))
        )
