...
(defmethod restore :channel [state parent]
           (let [r (do-restore state parent)]
             ...
             r))
(signal stream receive [_r _f {to :to  envelope :envelope content :content}]
        (rsync _r
               (let [p (parent-of _r)
                     droplet-address (--> matrice->incorporate _r p {:from (address-of _r) :to to :aspect (contents _r :name) :envelope envelope :content content})
                     ]
                 droplet-address)))
