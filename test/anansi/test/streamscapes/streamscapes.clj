...
(deftest streamscapes
  (let [m (receptor user nil "eric" nil)
    ...
    (testing "identity"
      (let [identity-address1 (s-> matrice->identify r {:identifiers {:email "eric@example.com" :ssn 987564321} :name "Eric"})
            identity-address2 (s-> matrice->identify r {:identifiers {:email "eric@otherexample.com" :ssn 123456789} :name "Eric"})
